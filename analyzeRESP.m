...
function [rr,rr_fft] = analyzeRESP(time,resp,plotsOn,thirty,compare)
    % INPUTS: 
    % time: elapsed time (seconds)
    % resp: output from pressure sensor (voltage)
    % plotsOn: true for plots, false for no plots
    % thirty: true for a 30 second interval, false for no analysis of this
    % interval
    % compare: true for comparison of rr and rr_FFT in 30 second increments
    % Editors note: I couldn't get compare to end up working and I am going
    % insane so I hope there's at least something of value in there
